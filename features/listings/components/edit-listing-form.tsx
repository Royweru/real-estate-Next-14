...
export const EditListingForm = ({
  ...
  const form = useForm<z.infer<typeof createListingSchema>>({
    ...
    defaultValues: {
      ...
      priceType:
        data?.priceType === "purchase" || data?.priceType === "rental"
          ? data.priceType
          : undefined,
  ...
  const onSubmit = async (values: z.infer<typeof createListingSchema>) => {
    try {
      ...
      console.log(values);
      const res = await axios.patch(`/api/listings/edit/${data?.id}`, values);
      if (res.status === 201) {
    ...
    <div className="w-full min-h-screen flex flex-col gap-y-6 p-4">
      <div className="flex flex-col gap-y-2">
        ...
        <p className="text-neutral-600">
          Make changes to your listing if you want to
        </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-4 gap-4 mt-4">
                        {field.value.length > 0 &&
                          field.value.map((img: { url: string }) => (
                            <div
